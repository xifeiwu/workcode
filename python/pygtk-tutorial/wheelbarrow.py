...
class WheelbarrowExample:
    ...
    def click(self, widget, event):
        if event.button == 1:
            ...
            self.drag_x =  event.x
            self.drag_y =  event.y
            print("position in click : %s * %s." % (self.drag_x, self.drag_y))
        elif event.button == 3:
            gtk.main_quit()
    ...
    def mousemove(self,widget,event):
        #print("position in mousemove : %s * %s" % (event.x, event.y))
        if self.drag:
            self.fixed.move(self.event_box,self.x+int(event.x-self.drag_x),self.y+int(event.y-self.drag_y))
            self.x, self.y = self.fixed.child_get(self.event_box,"x","y")
...
#        window.connect("button_press_event", self.button_clicked)
        self.window.show()
        self.x = 0
        self.y = 0
        # Now for the pixmap and the image widget
        pixmap, mask = gtk.gdk.pixmap_create_from_xpm_d(
            self.window.window, None, WheelbarrowFull_xpm)
        image = gtk.Image()
...
#        btnimg.show()
        ...
        # show the window
        print("Window's size : %s * %s", self.window.get_size())
