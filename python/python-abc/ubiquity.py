...
def install(frontend=None, query=False):
    ...
    """
    print("- in /usr/bin/ubiquity.install. frontend : %s" % (frontend))
    if frontend is None:
    ...
    print ("- in /usr/bin/ubiquity.install. distro : %s" % distro)
...
def acquire_lock():
    ...
    if 'DEBIAN_HAS_FRONTEND' not in os.environ:
        ...
        test_debconf = subprocess.Popen(
            ...
            universal_newlines=True)
        try:
            print('VERSION 2.0', file=test_debconf.stdin)
...
def main(oem_config):
    ...
    if not options.webcam:
        os.environ['UBIQUITY_NO_WEBCAM'] = '1'
...
if __name__ == '__main__':
    ...
    print("- /usr/bin/ubiquity.__main__. script_name : %s" % script_name)
